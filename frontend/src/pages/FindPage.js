...
const FindPage = () => {
  ...
  const sendEmail = async()=>{
    try{
      const result = await axios.post("https://6f2b-121-147-100-85.ngrok-free.app/teams/send-email",{
          email:email,
      headers: {
  ...
  const checkNum = async()=>{
    try{
      const result = await axios.post("https://6f2b-121-147-100-85.ngrok-free.app/teams/check-num",{
      ...
      <link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@48,400,0,0" />
        ...
        <div className='find_input'>
          ...
          <div style={{display:"flex",alignItems:"center",position:'relative'}}>
            ...
            <button className='btn_find_ok' onClick={sendEmail}>보내기</button>
          </div>
          <div style={{display:"flex",alignItems:"center",position:'relative'}}>
            <span style={{fontSize:"30px"}} className="material-symbols-outlined">lock</span>
            <input placeholder='인증번호 입력' onChange={(e)=>{setCheckNumber(e.target.value)}}/>
            <button className='btn_find_ok' onClick={checkNum}>확인</button>           
          </div>
          <div className='find_ment2' style={{margin:'0px',visibility:hidden}}>
          <div>사이트에 등록된 이메일 주소로 인증번호를 보내드렸습니다.</div>
          <div>인증번호를 입력하세요.</div>
          </div>
          <button className='btn_find_next' disabled={disabled} onClick={()=>{setNextBtn(true)}}>다음</button>
          </div></>
          :<FindPage2/>}
